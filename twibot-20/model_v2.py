...
class HGTDetector(nn.Module):
    def __init__(self, n_cat_prop=4, n_num_prop=5, des_size=768, tweet_size=768, num_neighbors=10,
                 embedding_dimension=128, word_vec=None, dropout=0.3):
        ...
        self.user_encoder_only_tweet = nn.Sequential(
            nn.Linear(embedding_dimension * self.num_neighbors, embedding_dimension),
            nn.LeakyReLU()
        )
    ...
    def forward(self, data: HeteroData):
        ...
        x_dict["user"] = self.user_encoder_only_tweet(x_dict["user"])

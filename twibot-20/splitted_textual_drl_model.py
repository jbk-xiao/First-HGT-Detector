...
class VAEEncoder(nn.Module):
    ...
    def __init__(self, weight):
        ...
        # ================ Encoder model ============= #
        # embedding_dim = word_vec_embedding_dim
        # output shape: (seq_len, batch, num_directions * hidden_size)
        self.encoder = nn.GRU(
            model_config.gru_embedding_dim, model_config.gru_hidden_dim, batch_first=True, bidirectional=True)
        # content latent embedding
        self.content_mu = nn.Linear(
            2 * model_config.gru_hidden_dim, model_config.content_hidden_dim)
        self.content_log_var = nn.Linear(
            2 * model_config.gru_hidden_dim, model_config.content_hidden_dim)
        # style latent embedding
        self.style_mu = nn.Linear(
            2 * model_config.gru_hidden_dim, model_config.style_hidden_dim)
    ...
    def forward(self, sequences, seq_lengths):
        ...
        generative_emb = torch.concat((sampled_style_emb, sampled_content_emb), dim=1)
        return content_emb_log_var, content_emb_mu, generative_emb, sampled_content_emb, sampled_style_emb, sequences, style_emb_log_var, style_emb_mu
...
class AdversarialVAE(nn.Module):
    ...
    def __init__(self, weight):
        ...
        # embedding_dim = word_vec_embedding_dim
        # output shape: (seq_len, batch, num_directions * hidden_size)
        self.vae_encoder = VAEEncoder(weight)
        # content latent embedding
        self.content_mu = nn.Linear(
            2*model_config.gru_hidden_dim, model_config.content_hidden_dim)
        self.content_log_var = nn.Linear(
        ...
        self.style_disc = nn.Linear(
            model_config.content_hidden_dim, model_config.num_style)
        self.content_disc = nn.Linear(
            model_config.style_hidden_dim, model_config.content_bow_dim)
        # =============== Classifier ============= #
        self.content_classifier = nn.Linear(
            model_config.content_hidden_dim, model_config.content_bow_dim)
        self.style_classifier = nn.Linear(
            model_config.style_hidden_dim, model_config.num_style)
        # =============== Decoder ================= #
        # Note: input embeddings are concatenated with the sampled latent vector at every step
        self.decoder = nn.GRUCell(
            model_config.gru_embedding_dim + model_config.generative_emb_dim, model_config.gru_hidden_dim)
        self.projector = nn.Linear(model_config.gru_hidden_dim, model_config.vocab_size)
    ...
    def forward(self, sequences, seq_lengths, style_labels, content_bow, iteration, last_epoch=None):
        """
        Args:
        ...
        Returns:
            ...
            vae_and_classifier_loss : consists of loss incurred by autoencoder, content and style
                                      classifiers
        """
        content_emb_log_var, content_emb_mu, generative_emb, sampled_content_emb, sampled_style_emb, sequences,\
            style_emb_log_var, style_emb_mu = self.vae_encode(sequences, seq_lengths)
        ...
        style_kl_loss = self.get_kl_loss(
            style_emb_mu, style_emb_log_var)
        if iteration < model_config.kl_anneal_iterations:
            style_kl_loss = self.get_annealed_weight(
                iteration, model_config.style_kl_lambda) * style_kl_loss
        # Content space
        content_kl_loss = self.get_kl_loss(
            content_emb_mu, content_emb_log_var)
        if iteration < model_config.kl_anneal_iterations:
            content_kl_loss = self.get_annealed_weight(
                iteration, model_config.content_kl_lambda) * content_kl_loss
        ...
        # ================ total weighted loss ========== #
        vae_and_classifier_loss = model_config.content_adversary_loss_weight * content_entropy_loss + \
            model_config.style_adversary_loss_weight * style_entropy_loss + \
            model_config.style_multitask_loss_weight * style_mul_loss + \
            model_config.content_multitask_loss_weight * content_mul_loss + \
    ...
    def vae_encode(self, sequences, seq_lengths):
        # pack the sequences to reduce unnecessary computations
        # It requires the sentences to be sorted in descending order to take
        # full advantage
        seq_lengths, perm_index = seq_lengths.sort(descending=True)
    ...
    def get_params(self):
        ...
        Returns:
            ...
            vae_and_classifiers_params       : parameters of the vae and classifiers
        """

        content_disc_params = self.content_disc.parameters()
        style_disc_params = self.style_disc.parameters()
        vae_and_classifiers_params = list(self.encoder.parameters()) + list(self.decoder.parameters()) + \
            list(self.style_classifier.parameters()) + \
            list(self.content_classifier.parameters())

        return content_disc_params, style_disc_params, vae_and_classifiers_params
    ...
    def get_content_disc_loss(self, content_disc_preds, content_bow):
        ...
        smoothed_content_bow = content_bow * \
            (1-model_config.label_smoothing) + \
            model_config.label_smoothing/model_config.content_bow_dim
    ...
    def get_style_disc_loss(self, style_disc_preds, style_labels):
        ...
        smoothed_style_labels = style_labels * \
            (1-model_config.label_smoothing) + \
            model_config.label_smoothing/model_config.num_style
        # calculate cross entropy loss
    ...
    def get_entropy_loss(self, preds):
        ...
        input distribution
        """
        return torch.mean(torch.sum(preds * torch.log(preds + model_config.epsilon), dim=1))
    ...
    def get_annealed_weight(self, iteration, lambda_weight):
        ...
        """
        return (math.tanh(
            (iteration - model_config.kl_anneal_iterations * 1.5) /
            (model_config.kl_anneal_iterations / 3))
            + 1) * lambda_weight
    ...
    def generate_sentences(self, input_sentences, latent_emb, inference=False):
        ...
        if not inference:
            ...
            latent_emb = latent_emb.unsqueeze(1).repeat(
                1, model_config.max_seq_len+1, 1)
            ...
            output_sentences = torch.zeros(
                model_config.max_seq_len, input_sentences.shape[0], model_config.vocab_size, device=input_sentences.device)
            # initialize hidden state
            hidden_states = torch.zeros(
                input_sentences.shape[0], model_config.gru_hidden_dim, device=input_sentences.device)
            # generate sentences one word at a time in a loop
            for idx in range(model_config.max_seq_len):
        ...
        # if inference mode is on
            ...
            output_sentence = torch.zeros(
                model_config.max_seq_len, 1, device=latent_emb.device)
            with torch.no_grad:

...
tweet_embs = []
for tweet_sequence, seq_length in tqdm(gen_iter, desc='Generating tweet emb...', total=generate_dataset.__len__()):
    content_emb, style_emb = vae_model.get_style_content_emb(
        tweet_sequence.unsqueeze(0).to(device), seq_length.unsqueeze(0)
    )
